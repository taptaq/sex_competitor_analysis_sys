...
import { Send, Sparkles, Loader2, Target, ShieldCheck, DollarSign, AlertCircle } from 'lucide-react';
...
const StrategyAdvisor: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const advice = await getStrategyAdvice(concept);
      setResult(advice);
    } catch (err) {
      console.error(err);
      setResult({
        differentiation: "由于连接压力测试引擎失败，无法生成报告。请检查网络或 API 状态。",
        compliance: "服务暂时不可用。",
        pricing: "无法计算。"
      });
    } finally {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-12">
      <div className="text-center space-y-4">
        ...
        <h1 className="text-3xl font-extrabold text-gray-900">创业策略压力测试员</h1>
        <p className="text-gray-500">抛弃幸存者偏差，用真实行业中位数数据审视你的想法。</p>
      </div>

      <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
        <form onSubmit={handleSubmit} className="space-y-4">
          <label className="block text-sm font-bold text-gray-700">描述你的新产品概念或商业计划...</label>
      ...
      {result && (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4">
            ...
            <p className="text-sm text-gray-600 leading-relaxed">{result.differentiation}</p>
          </div>
          ...
          <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4">
            ...
            <p className="text-sm text-gray-600 leading-relaxed">{result.compliance}</p>
          </div>
          ...
          <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4">
            ...
            <p className="text-sm text-gray-600 leading-relaxed">{result.pricing}</p>
          </div>
        </div>
      )}
      ...
      {!result && !loading && (
        <div className="flex flex-col items-center justify-center py-12 text-gray-400 space-y-4">
          ...
          <p className="text-sm italic">输入你的想法，接受最真实的市场残酷审讯。</p>
        </div>
      )}
    </div>
  );
